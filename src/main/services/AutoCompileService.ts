 ...
 */
export class AutoCompileService extends EventEmitter {
  ...
  private async executeCompile(projectId: string): Promise<void> {
    if (!this.isEnabled) {
  ...
  private cancelJob(projectId: string): void {
   ...
   */
  getStatus(): { enabled: boolean; delay: number; activeJobs: number } {
    return {
