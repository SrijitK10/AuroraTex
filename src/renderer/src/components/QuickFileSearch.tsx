...
const HighlightedText: React.FC<{ text: string; matches: number[] }> = ({ text, matches }) => {
  ...
  matches.forEach((index, i) => {
    ...
    if (index > lastIndex) {
      parts.push(
        <span key={`text-${i}`} className="text-gray-600 dark:text-gray-400">
    ...
    parts.push(
      <span key={`match-${i}`} className="bg-yellow-200 dark:bg-yellow-500/30 text-gray-900 dark:text-yellow-100 font-medium rounded-sm">
        {text[index]}
      </span>
    );
  ...
  if (lastIndex < text.length) {
    parts.push(
      <span key="text-end" className="text-gray-600 dark:text-gray-400">
...
export const QuickFileSearch: React.FC<QuickFileSearchProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-gray-900/50 dark:bg-black/50 backdrop-blur-sm flex items-start justify-center pt-20 z-50 transition-all">
      <div
        ref={modalRef}
        className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-md rounded-2xl shadow-2xl border border-white/20 dark:border-gray-700/50 w-full max-w-2xl mx-4 max-h-96 flex flex-col"
      >
        {/* Header */}
        <div className="p-4 glass-header border-b border-gray-200/50 dark:border-gray-800/50 rounded-t-2xl shrink-0">
          <div className="flex items-center space-x-3">
            ...
            />
            <div className="text-xs text-gray-400 dark:text-gray-500 hidden sm:flex items-center space-x-1">
              <kbd className="px-1.5 py-0.5 bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm text-gray-500 dark:text-gray-400 rounded-md text-xs">↑↓</kbd>
              <span>navigate</span>
              <kbd className="px-1.5 py-0.5 bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm text-gray-500 dark:text-gray-400 rounded-md text-xs">Tab</kbd>
              <span>next</span>
              <kbd className="px-1.5 py-0.5 bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm text-gray-500 dark:text-gray-400 rounded-md text-xs">↵</kbd>
              <span>select</span>
              <kbd className="px-1.5 py-0.5 bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm text-gray-500 dark:text-gray-400 rounded-md text-xs">esc</kbd>
              <span>close</span>
            </div>
          </div>
        </div>
            ...
            <div className="py-2">
              {searchResults.map((file, index) => (
                <button
                  ...
                  className={`w-full px-4 py-3 text-left hover:bg-gray-50/50 dark:hover:bg-gray-800/50 flex items-center space-x-3 border-l-4 transition-all duration-150 text-gray-900 dark:text-gray-100 ${
                    index === selectedIndex
                      ? 'bg-blue-50 dark:bg-blue-900/20 border-l-blue-500 ring-1 ring-blue-500/20 dark:ring-blue-400/20 shadow-sm'
        ...
        {/* Footer */}
        {searchResults.length > 0 && (
          <div className="px-4 py-2 border-t border-gray-200/50 dark:border-gray-800/50 text-xs text-gray-500 dark:text-gray-400 bg-gray-50/50 dark:bg-gray-800/50 backdrop-blur-sm rounded-b-2xl shrink-0">
