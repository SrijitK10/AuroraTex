...
class SnippetService {
    constructor() {
        this.snippetsDir = (0, path_1.join)((0, os_1.homedir)(), '.myoverleaf');
        this.snippetsFile = (0, path_1.join)(this.snippetsDir, 'snippets.json');
    }
    async initialize() {
        // Ensure directory exists
        if (!(0, fs_1.existsSync)(this.snippetsDir)) {
            await (0, promises_1.mkdir)(this.snippetsDir, { recursive: true });
        }
        // Create default snippets if file doesn't exist
        if (!(0, fs_1.existsSync)(this.snippetsFile)) {
    ...
    async list() {
        try {
            ...
            const content = await (0, promises_1.readFile)(this.snippetsFile, 'utf-8');
    ...
    async search(query) {
        ...
        return snippets.filter(snippet => snippet.name.toLowerCase().includes(lowercaseQuery) ||
...
\\end{verbatim}`,
        ...
        await (0, promises_1.writeFile)(this.snippetsFile, JSON.stringify(snippetsWithIds, null, 2));
    }
}
